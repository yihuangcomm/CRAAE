...
result_folder = cfg.OUTPUT_DIR +"_"+ cfg.DATASET.PATH_out +"_" + cfg.DATASET.calibrate + "_" + "_".join([str(x) for x in selected_classes]) + "acc10"+ "result.csv"
...
# print(settings)
def f(setting):
    import train_CRAAE_nw
    import novelty_detector_nw

    fold_id = setting['fold'] 
    inlier_classes = setting['digit']
    print("training classes {}".format(inlier_classes))
    # train_CRAAE_nw.train(fold_id, inlier_classes, inlier_classes)
    
    res = novelty_detector_nw.main(fold_id, inlier_classes, inlier_classes, classes_count, mul)
    return res
